...
# Single-word
for line in open("bias_data/WNC_edit/single_test.txt", "r"):
...
with open("inference/stats_source_copy_single.txt", "w") as f:
    ...
    f.write("=============\n")
    for i, (p, g) in tqdm(enumerate(zip(pred_data, gold_data)), desc="Write source_copy_single..."):
...
with open("inference/stats_target_copy_single.txt", "w") as f:
    ...
    f.write("=============\n")
    for i, (p, g) in tqdm(enumerate(zip(gold_data, gold_data)), desc="Write target_copy_single..."):
        f.write("{} - {:>6.2f}\n".format(str(i + 1).zfill(4), get_bleu(p, g)))
    f.write("=============\n")

# Multi-word
for line in open("bias_data/WNC_edit/multi_test.txt", "r"):
    pred = line.split("\t")[1].rstrip()
    gold = line.split("\t")[2].rstrip()
    pred_data.append(pred)
    gold_data.append(gold)
...
with open("inference/stats_source_copy_multi.txt", "w") as f:
    ...
    f.write("ACC:  {:>7,.4f}\n".format(np.mean(hits_source)))
    f.write("=============\n")
    for i, (p, g) in tqdm(enumerate(zip(pred_data, gold_data)), desc="Write source_copy_multi..."):
        f.write("{} - {:>6.2f}\n".format(str(i + 1).zfill(4), get_bleu(p, g)))
    f.write("=============\n")
...
with open("inference/stats_target_copy_multi.txt", "w") as f:
    ...
    f.write("ACC:  {:>7,.4f}\n".format(np.mean(hits_target)))
    f.write("=============\n")
    for i, (p, g) in tqdm(enumerate(zip(gold_data, gold_data)), desc="Write target_copy_multi..."):
        f.write("{} - {:>6.2f}\n".format(str(i + 1).zfill(4), get_bleu(p, g)))
    f.write("=============\n")
