...
# Read sentences
for line in open(os.path.join(source_path, "WNC/biased.word.train"), "r"):
...
with open(os.path.join(target_path, "WNC_word/train.txt"), "w") as train_file:
    with open(os.path.join(target_path, "WNC_word/train.label"), "w") as train_label_file:
        train_file.write("\n".join(train_set) + "\n")
        train_label_file.write("neutral\n" * len(train_set))

with open(os.path.join(target_path, "WNC_word/dev.txt"), "w") as dev_file:
    with open(os.path.join(target_path, "WNC_word/dev.label"), "w") as dev_label_file:
        dev_file.write("\n".join(dev_set) + "\n")
        dev_label_file.write("neutral\n" * len(dev_set))

with open(os.path.join(target_path, "WNC_word/test.txt"), "w") as test_file:
    with open(os.path.join(target_path, "WNC_word/test.label"), "w") as test_label_file:
        test_file.write("\n".join(test_set) + "\n")
